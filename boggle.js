...
// }
function find(board, word) {
  function _find(board, y, x, str, visited) {
    if (str.length === 0) {
      return true;
    }
    if (
    ...
    if (_find(board, y - 1, x, str.slice(1), visited)) return true;
    if (_find(board, y + 1, x, str.slice(1), visited)) return true;
    if (_find(board, y, x - 1, str.slice(1), visited)) return true;
    if (_find(board, y, x + 1, str.slice(1), visited)) return true;
  }

  for (let i = 0; i < board.length; i++) {
    for (let j = 0; j < board[i].length; j++) {
      if (_find(board, i, j, word, new Set())) return true;
    }
  }
